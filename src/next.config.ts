...
const withPWA = withPWAInit({
  ...
  runtimeCaching: [
    ...
    {
      // Strategy: Cache First for all other navigations and assets
      // This is the most important rule for offline functionality.
      // It caches all navigation requests and static assets.
      urlPattern: ({ request }) => {
        return request.mode === 'navigate' || request.destination === 'style' || request.destination === 'script' || request.destination === 'image';
      },
      handler: 'CacheFirst',
      options: {
